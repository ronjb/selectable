...
class SelectionState {
  ...
  ///
  bool containsPoint(Offset point) => _cachedRects?.containsPoint(point) ?? false;
  ...
  void update(
    ...
    //dmPrint('SelectionState.update(selectionPt: $selectionPt, handleType: $handleType)');
    ...
    // Local func for collecting all the descendant render paragraphs of a render object.
    bool collectParagraphs(Object ro) {
      ...
      if (rp != null) {
        final paragraph = SelectionParagraph.from(rp, ancestor: renderObject, index: index);
        if (paragraph != null) {
          ...
          //dmPrint('Adding paragraph with rect: $rect, range: $trimmedSel, text: "$text"\n');
    ...
    // Local func to cache the index of the paragraph that contains the selection point, if any.
    ...
    // Local func that returns `true` if the selection point is in, to the left of, or to the
    // right of the given [paragraph].
    ...
    // Iterate through the render paragraphs, collecting the selected `rects` and a string
    // buffer with the selected string to support `Copy`.
    //
    // Note, this algorithm assumes the render paragraphs are in the order the text should
    // be read, which generally is the case, but in some widgets, such as the Stack widget,
    // care needs to be taken to ensure the child text widgets are ordered correctly.
    //
    final rects = <Rect>[];
    ...
    for (var i = 0; i < paragraphs.length; i++) {
      ...
      if (isSelectingWord) {
        ...
        if (anchor?.containsPoint(selPt) ?? false) {
          ...
          buff.write(paragraph.text.substring(anchor.textSel.start, anchor.textSel.end));
          break; //------------------------------------->
        }
      } else {
        //
        ...
        if (_leftAnchor == null || _rightAnchor == null) {
          ...
          // If the selection point is above or to the left of this paragraph, set `anchor`
          // to the first word in the paragraph.
          ...
          if (anchor == null && selectionPtIsInOrNextTo(paragraph)) {
            anchor = paragraph.anchorAtPt(selPt, onlyIfInRect: true, trim: false);
          }

          // If the selection point is in, to the right of, or below the paragraph, but above
          // the next paragraph (if any), set `anchor` to the last word in the paragraph.
          if (anchor == null &&
              selPt.dx > paragraph.rect.left &&
              selPt.dy > paragraph.rect.top &&
              (paragraphContainingSelectionPt() == i ||
                  (paragraphContainingSelectionPt() == -1 &&
                      (i == paragraphs.length - 1 || selPt.dy < paragraphs[i + 1].rect.top)))) {
            anchor = paragraph.anchorAtCharIndex(paragraph.trimmedSel.end - 1, trim: false);
            assert(anchor != null);
          }
        ...
        if (_leftAnchor != null && _leftAnchor!.paragraph <= i) {
          ...
          final start =
              (isLeftAnchorParagraph ? _leftAnchor!.textSel.start : paragraph.trimmedSel.start);
          ...
          if (ts != null) {
            ...
            // Append the text to buff, adding zero or more carriage returns depending position
            // of this paragraph to the next one.
            final j = i + 1;
            final paragraphText = paragraph.text.substring(ts.start, ts.end);
            final paragraphBottom = selectionRects.last.bottom;
            final nextParagraphTop = paragraphs[j < paragraphs.length ? j : i].rect.top;
            if (isRightAnchorParagraph || nextParagraphTop < paragraphBottom) {
              // No carriage return: next paragraph is not below this one.
              buff..write(paragraphText)..write(' ');
            } else if (nextParagraphTop < paragraphBottom + selectionRects.first.height) {
              // One carriage return: next paragraph is on the next line.
              buff.writeln(paragraphText);
            } else {
              // Two carriage returns: next paragraph is greater than a line below.
              buff..writeln(paragraphText)..writeln();
            }
          }
    ...
    if (rects.isEmpty || _leftAnchor == null || _rightAnchor == null) {
      ...
      endTaggedText = _rightAnchor!.taggedTextWithParagraphs(paragraphs, end: true);

      // Cache the paragraphs, stripping out the RenderTextMixin objects.
      _cachedParagraphs = paragraphs
          .map((p) => SelectionParagraph(
              rp: null, rect: p.rect, index: p.index, text: p.text, trimmedSel: p.trimmedSel))
          .toList();
  ...
  ///
  void _autoscroll(ScrollController? scrollController, RenderObject renderObject, Offset? dragPt,
      double maxY, double topOverlayHeight) {
    ...
    final renderObjScrollPos = renderObject.getTransformTo(vp).getTranslation().y;
    ...
    if (scrollOffset > -topOverlayHeight &&
      ...
      scrollDelta = y - (scrollOffset + autoscrollAreaHeight + topOverlayHeight);
    } else if (y > scrollOffset + viewportExtent - autoscrollAreaHeight) {
      scrollDelta = y - (scrollOffset + viewportExtent - autoscrollAreaHeight);
    }

    if (scrollDelta != 0.0) {
      final newScrollOffset = min(renderObjectBottom - viewportExtent + 100.0,
          max(-renderObjectTop, scrollOffset + (scrollDelta * scrollDistanceMultiplier)));
  ...
  List<Widget> buildSelectionControls(
    ...
    // If showing the popup menu, dragging has stopped, so reset `_areAnchorsSwapped`.
    ...
    final _leftOffset = controls!.getHandleAnchor(TextSelectionHandleType.left, leftLineHeight);
    final _rightOffset = controls!.getHandleAnchor(TextSelectionHandleType.right, rightLineHeight);

    final leftPt = Offset(_leftHandlePt!.dx - _leftOffset.dx, _leftHandlePt!.dy - _leftOffset.dy);
    final rightPt = Offset(_rightHandlePt!.dx - _rightOffset.dx, _rightHandlePt!.dy);

    final leftSize = controls!.getHandleSize(leftLineHeight);
    final rightSize = controls!.getHandleSize(rightLineHeight);

    final leftRect =
        Rect.fromLTWH(leftPt.dx, leftPt.dy, leftSize.width, leftSize.height).inflate(20);
    final rightRect =
        Rect.fromLTWH(rightPt.dx, rightPt.dy, rightSize.width, rightSize.height).inflate(20);

    final isShowingPopupMenu = (showPopupMenu && !isScrolling);
    // dmPrint('SelectionState.buildSelectionControls isShowingPopupMenu == $isShowingPopupMenu');
    // dmPrint('buildSelectionControls, showPopupMenu = $showPopupMenu, isScrolling = $isScrolling');
    ...
    return [
      Positioned.fromRect(
        ...
        child: _SelectionHandle(
          ...
          child: controls!.buildHandle(context, TextSelectionHandleType.left, leftLineHeight),
          mainKey: mainKey,
        ),
      ),
      Positioned.fromRect(
        ...
        child: _SelectionHandle(
          ...
          child: controls!.buildHandle(context, TextSelectionHandleType.right, rightLineHeight),
          mainKey: mainKey,
        ),
      ),
      AnimatedOpacity(
...
class _PopupMenuState extends State<_PopupMenu> {
  ...
  Widget build(BuildContext context) {
    if (_menu == null) {
      ...
      // [viewport] is the rectangle that can be seen, in render object coordinates, which
      // defaults to the render object rect.
      Rect? viewport =
          Rect.fromLTWH(0, 0, widget.constraints.maxWidth, widget.constraints.maxHeight);
      ...
      if (widget.scrollController?.hasClients ?? false) {
        ...
        if (vp != null) {
          ...
          final viewportExtent =
              widget.scrollController!.position.viewportDimension - widget.topOverlayHeight;
          viewport =
              Rect.fromLTWH(0, scrollOffset, viewport.width, viewportExtent).intersect(viewport);
          if (viewport.height < 50) viewport = null;
        }
      }

      if (viewport != null) {
        _menu = widget.controls
            .buildPopupMenu(context, viewport, widget.selectionRects, widget.selectionDelegate);
      } else {
        _menu = Container();
      }
    }
